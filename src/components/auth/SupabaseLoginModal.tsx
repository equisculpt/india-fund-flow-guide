...
import { User, Building2, Eye, EyeOff, Shield } from "lucide-react";
...
const SupabaseLoginModal = ({ isOpen, onClose }: SupabaseLoginModalProps) => {
  ...
  const { signIn, signUp, signInWithGoogle, twoFactorRequired, verifyPAN, completeTwoFactor } = useSupabaseAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        // If 2FA is required, the modal will show the PAN input
      }
    } catch (error: any) {
      toast({
        title: "Error",
  ...
  const handlePANVerification = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          ...
          description: result.error || "Failed to verify PAN number",
          variant: "destructive",
        });
      }
    } catch (error: any) {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <p className="text-sm text-gray-600">
              Please enter your PAN number to complete the login process.
            </p>
